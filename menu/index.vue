<template>
  <nav class="space-y-1 transition-all duration-300 overflow-hidden" :class="[isExpanded ? 'w-full' : 'w-10']">
    <template v-for="(item, index) in menuItems" :key="index">
      <!-- 普通菜单项 -->
      <template v-if="!('isSection' in item)">
        <div class="relative" @mouseenter="showTooltip(item)" @mouseleave="hideTooltip">
          <router-link :to="item.path" class="flex items-center h-10 text-[15px] rounded-lg group relative" :class="[
      ...
      <template v-else>
        <div class="pt-2 space-y-1">
          <div class="h-7 flex items-center cursor-pointer relative" @mouseenter="showTooltip(item)"
            @mouseleave="hideTooltip" @click="toggleSection(item.title)">
            <div class="w-10 flex items-center justify-center flex-shrink-0">
              <ChevronRight class="w-4 h-4 text-gray-400 transition-transform duration-300"
          ...
          <div class="relative">
            <Transition enter-active-class="transition-all duration-300 ease-out"
              ...
              <div v-show="expandedSections[item.title]" class="overflow-hidden">
                <template v-for="subItem in item.items" :key="subItem.path">
                  <div class="relative" @mouseenter="showTooltip(subItem)" @mouseleave="hideTooltip">
                    <router-link :to="subItem.path" class="flex items-center h-10 text-[15px] rounded-lg group relative"
                      :class="[
